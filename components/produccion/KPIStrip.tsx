...
import { Sprout, Leaf, Flower2, CheckCircle2, BarChart2 } from 'lucide-react';
import { KPIS_PROD_PAGE } from './mock-data';

const ICONS = [Sprout, Leaf, Flower2, CheckCircle2, BarChart2];
...
function Spark({ data, up }: { data: number[]; up: boolean }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} fill="none">
      ...
      <polyline points={`0,${h} ${pts} ${w},${h}`} fill={`url(#pg${up})`} opacity={0.18}/>
    </svg>
  );
}

export function KPIStripProduccion() {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
      {KPIS_PROD_PAGE.map((k, i) => {
        const Icon = ICONS[i];
        return (
