...
import { ShoppingCart, Loader2, Clock, CheckCircle2, Wallet, PiggyBank } from 'lucide-react';
import { KPIS_ORD } from './mock-data';

const ICONS = [ShoppingCart, Loader2, Clock, CheckCircle2, Wallet, PiggyBank];
...
function Spark({ data, up }: { data: number[]; up: boolean }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} fill="none">
      <defs>
        <linearGradient id={`og${up}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={c}/><stop offset="100%" stopColor={c} stopOpacity="0"/>
        </linearGradient>
      </defs>
      <polyline points={pts} stroke={c} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" opacity={0.8}/>
      <polyline points={`0,${h} ${pts} ${w},${h}`} fill={`url(#og${up})`} opacity={0.18}/>
    </svg>
  );
}

export function KPIStripOrd() {
  return (
