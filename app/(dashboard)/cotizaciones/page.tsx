      ...
      <div className="rounded-lg border border-border bg-card p-3 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-green-700">{formatAmount(cot.total)} <CurrencyFlag currency={currency} /></span>
            ...
            {cot.items.map((item) => (
              <div key={item.id} className="flex items-center gap-2 py-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{item.cantidad} {item.unidad} × {formatAmount(item.precioUnitario)} <CurrencyFlag currency={currency} /></p>
                </div>
                <span className="text-xs font-medium shrink-0">{formatAmount(item.subtotal)} <CurrencyFlag currency={currency} /></span>
              </div>
            ))}
